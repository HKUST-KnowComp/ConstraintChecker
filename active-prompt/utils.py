...
# ckbp_prompt_template = "Is the statement '{}' plausible?"
ckbp_prompt_template = "Judge the following statement if it's likely to occur, only answer 'True' or 'False': {}"
...
TIMEOUT_FAILURE = 1
def chatgpt(prompt, max_tokens=5, temperature=0):
    resp = None
    done = False
    ...
    if args.dataset == "gsm8k":
        ...
        with open(args.dataset_path) as f:
            ...
            for line in json_data:
                ...
                questions.append(q)
                answers.append(a)
    elif args.dataset in ("ckbp2"):
        data_df = pd.read_csv(args.dataset_path)
        datas = data_df['assertion'].tolist()
        labels = data_df['label'].tolist()
        for question, label in zip(datas, labels):
            questions.append(ckbp_prompt_template.format(question))
...
def answer_extraction(args, responses):
    ...
    elif args.dataset in ("strategyqa", "coin_flip", "ckbp2"):
        temp = temp.lower()
        temp = re.sub("\"|\'|\n|\.|\s|\:|\,"," ", temp)
        temp = temp.split(" ")
